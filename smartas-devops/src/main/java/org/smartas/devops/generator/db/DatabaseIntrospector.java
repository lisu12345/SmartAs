...
import static org.smartas.devops.generator.util.JavaBeansUtil.getCamelCaseString;
import static org.smartas.devops.generator.util.JavaBeansUtil.getValidPropertyName;
import static org.smartas.devops.generator.util.Messages.getString;
import static org.smartas.devops.generator.util.StringUtility.composeFullyQualifiedTableName;
import static org.smartas.devops.generator.util.StringUtility.isTrue;
import static org.smartas.devops.generator.util.StringUtility.stringContainsSQLWildcard;
import static org.smartas.devops.generator.util.StringUtility.stringContainsSpace;
import static org.smartas.devops.generator.util.StringUtility.stringHasValue;
...
import org.smartas.devops.generator.java.JavaReservedWords;
...
import org.smartas.devops.generator.meta.FullyQualifiedTable;
...
public class DatabaseIntrospector {
   ...
   * @param table the table
   */
  private void reportIntrospectionWarnings(IntrospectedTable introspectedTable, TableConfig tableConfiguration,
      FullyQualifiedTable table) {
    // make sure that every column listed in column overrides
    // actually exists in the table
    /*
     * for (ColumnOverride columnOverride : tableConfiguration.getColumnOverrides()) { if
     * (introspectedTable.getColumn(columnOverride.getColumnName()) == null) {
     * warnings.add(getString("Warning.3", //$NON-NLS-1$ columnOverride.getColumnName(),
     * table.toString())); } }
     */
     ...
     * warnings.add(getString("Warning.4", //$NON-NLS-1$ string, table.toString())); }
     * 
     * GeneratedKey generatedKey = tableConfiguration.getGeneratedKey(); if (generatedKey != null &&
     * introspectedTable.getColumn(generatedKey.getColumn()) == null) { if
     * (generatedKey.isIdentity()) { warnings.add(getString("Warning.5", //$NON-NLS-1$
     * generatedKey.getColumn(), table.toString())); } else { warnings.add(getString("Warning.6",
     * //$NON-NLS-1$ generatedKey.getColumn(), table.toString())); } }
     */
   ...
   */
  public List<IntrospectedTable> introspectTables(TableConfig tc) throws SQLException {
   ...
   */
  private void removeIgnoredColumns(TableConfig tc, Map<ActualTableName, List<IntrospectedColumn>> columns) {
   ...
   * @param columns the columns
   */
  private void calculateExtraColumnInformation(TableConfig tc,
      Map<ActualTableName, List<IntrospectedColumn>> columns) {
    StringBuilder sb = new StringBuilder();
    Pattern pattern = null;
    String replaceString = null;
    /*
     * if (tc.getColumnRenamingRule() != null) { pattern =
     * Pattern.compile(tc.getColumnRenamingRule().getSearchString()); replaceString =
     * tc.getColumnRenamingRule().getReplaceString(); replaceString = replaceString == null ? "" :
    ...
    for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
      for (IntrospectedColumn introspectedColumn : entry.getValue()) {
        ...
        if (fullyQualifiedJavaType != null) {
          ...
          introspectedColumn
              .setJdbcTypeName(javaTypeResolver.calculateJdbcTypeName(introspectedColumn));
        } else {
          ...
          /*
           * ColumnOverride co = tc.getColumnOverride(introspectedColumn.getActualColumnName()); if
           * (co != null) { if (stringHasValue(co.getJavaType()) &&
           * stringHasValue(co.getJavaType())) { warn = false; } }
           */
        ...
        /*
         * if (context.autoDelimitKeywords()) { if
   ...
   * @param columns the columns
   */
  private void calculateIdentityColumns(TableConfig tc,
      Map<ActualTableName, List<IntrospectedColumn>> columns) {
  ...
  /**
   * This method returns a Map<ActualTableName, List<ColumnDefinitions>> of columns returned from
   ...
   * @throws SQLException the SQL exception
   */
  private Map<ActualTableName, List<IntrospectedColumn>> getColumns(TableConfig tc) throws SQLException {
    String localCatalog;
    String localSchema;
   ...
   * @return the list
   */
  private List<IntrospectedTable> calculateIntrospectedTables(TableConfig tc,
      Map<ActualTableName, List<IntrospectedColumn>> columns) {
    boolean delimitIdentifiers = tc.isDelimitIdentifiers() || stringContainsSpace(tc.getCatalog())
        || stringContainsSpace(tc.getSchema()) || stringContainsSpace(tc.getTableName());
    ...
    for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
      ...
      // Table introspectedTable = new Table(tc, table, context);
  ...
  /**
   * This method calls database metadata to retrieve some extra information about the table such as
