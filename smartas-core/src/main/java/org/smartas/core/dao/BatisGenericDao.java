...
public class BatisGenericDao<T extends POJO, PK extends Serializable> extends SqlSessionDaoSupport
    implements
      GenericDao<T, PK> {
  ...
  public List<T> select(final Page page) throws DataAccessException {
    return super.getSqlSession().selectList(selectStatement(clazz),
        new RowBounds(page.getFirstResult(), page.getMaxResult()));
  }
  ...
  protected final List<T> select(String statement, final int offset, final int limit)
      throws DataAccessException {
    return this.select(statement, null, offset, limit);
  }

  protected final List<T> select(String statement, Object params, final int offset, final int limit)
      throws DataAccessException {
    return super.getSqlSession().selectList(statement(clazz, statement), params,
  ...
  @Override
  public List<T> selectAll(QueryFilter filter) {
    return null;
  }

  @Override
  public List<T> select(QueryFilter filter, Page page) {
    return null;
  }
}
